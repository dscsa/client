...
export class inventory {
  ...
  activate(params) {
    ...
    this.db.user.session.get().then(session => {
      ...
      this.db.transaction.query('currently-pended-by-group-priority-generic', {group_level: 5, startkey:[this.account._id], endkey:[this.account._id, {}]})
      .then(res => {
        this.setPended(res.rows.map(row => row.key[4]))
