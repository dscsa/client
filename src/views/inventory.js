...
import {InventoryItemsList} from 'client/src/resources/v3/InventoryItemsList';
...
export class inventory {
    ...
    constructor(db, router) {
        ...
        this.inventoryItemsList = new InventoryItemsList();
...
console.log('syncPended');
    ...
    toggleCheck(transaction) {
        ...
        //this.updateFilterCountsAndSetCheck(transaction, !transaction.isChecked)
    }
    ...
    setVisibleChecks(isChecked) {
        ...
        for (let transaction of filtered)
            this.updateFilterCountsAndSetCheck(transaction, isChecked)
    ...
    updateFilterCountsAndSetCheck(transaction, isChecked) {

        //this.inventoryItemsList.setSelectedStatusById(transaction.id, isChecked);
...
console.log(transactions);
    ...
    selectInventory(type, key, limit) {
        ...
        if (type == 'bin' && key.length == 3) {
            ...
            v3.getRequest('inventory/available/' + key).onSuccess((response) => {
                console.log(response);
                this.inventoryItemsList.setItems(response);
            });
        ...
        //this code will cause an infinite loop of some sort - very hard to trace.
    ...
    updateSelected(updateFn) {
        ...
        for (let i = length - 1; i >= 0; i--) {
            ...
            this.updateFilterCountsAndSetCheck(transaction, false)
    ...
    getSelectedTransactionIds() {
        ...
        for (let transaction of Object.values(this.selectedTransactions)) {
    ...
    pendInventory() {
        ...
        let toPend = []
        let repackQty = this.pendToQty;
        let pended_obj = {_id: new Date().toJSON(), user: this.user, repackQty: repackQty, group: this.pendToId};
        let ids = this.getSelectedTransactionIds();

        let request = v3.postRequest('inventory/pend', {
            inventory_ids: ids,
        ...
        request.onSuccess(() => {
            ids.forEach((id) => {
                let inventoryItem = this.inventoryItemsList.removeItemById(id);
                this.updateFilterCountsAndSetCheck(inventoryItem, false);
                //todo: figure out what's going on below
            });
        ...
        return;
        let transactions_in_group = this.extractTransactions(this.pendToId, '') //gives an array of transactions that need to have priority toggled, then saved
...
console.log(this.inventoryItemsList);
    ...
    printLabels(transactions) {
        ...
        for (let item of Object.values(transactions)){
            const label = [
                `<p style="page-break-after:always; white-space:nowrap">`,
                `<strong>${item.productName}</strong>`,
                item.createdAt.slice(2, -1),
                `Ndc ${item.drugId}`,
                `Exp ${item.expirationDate.slice(0, 7)}`,
                `Bin ${item.bin}`,
                `Qty ${item.quantity}`,
    ...
    saveAndReconcileTransaction(transaction) {
        ...
        this.db.transaction.get(transaction._id).then(repack => {
            ...
            return this.db.transaction.query('next.transaction._id', {
                key: [this.account._id, transaction._id],
                include_docs: true
            }).then(res => {
    ...
    setExcessQty() {
     ...
     exportCSV() {
        ...
        const request = v3.getRequest('inventory/available');

        request.onSuccess((data) => {
            this.csv.fromJSON(name, data.map(item => {
                console.log(item);
                return {
                    ...
                    'bin': item.bin, //key 9 is sortedBin
                    '_id': item.id
                }
            }))
        });
        ...
        transactions = transactions.filter((transaction, i) => {
            ...
            if (!expFilter[isExp]) {
                expFilter[isExp] = {
                    ...
                    count: 0,
                    qty: 0
                } //if someone search for 'A00*' show only the expired items by default (this will help data entry people purging expireds)
            }
            ...
            if (!expFilter[isExp].isChecked) {
                ...
                return inventory.prototype.updateFilterCountsAndSetCheck.call({filter}, transaction, false)
            }
