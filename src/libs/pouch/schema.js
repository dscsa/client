function pouchSchema(pouchModel, microSecond, methods = {}) {
  ...
  let db = {
    ...
    drug:pouchModel()
      .ensure().rules(drug)
      .ensure('_id').required().pattern(/\d{4}-\d{4}|\d{5}-\d{3}|\d{5}-\d{4}|\d{5}-\d{5}|\.S[ICRP]-[a-z]{2,7}-[a-zA-Z0-9]{15}/)
    ...
    transaction:pouchModel()
      ...
      .ensure('drug._id').required().pattern(/Unspecified|\d{4}-\d{4}|\d{5}-\d{3}|\d{5}-\d{4}|\d{5}-\d{5}|\.S[ICRP]-[a-z]{2,7}-[a-zA-Z0-9]{15}/)
