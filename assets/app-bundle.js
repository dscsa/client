...
define('views/inventory',['exports', 'aurelia-framework', 'libs/pouch', 'aurelia-router'], function (exports, _aureliaFramework, _pouch, _aureliaRouter) {
  ...
  exports.drugNameValueConverter = exports.filterValueConverter = exports.numberValueConverter = exports.dateValueConverter = exports.inventory = undefined;
  ...
  var inventory = exports.inventory = (_dec = (0, _aureliaFramework.inject)(_pouch.Db, _aureliaRouter.Router), _dec(_class = function () {
    ...
    inventory.prototype.activate = function activate(params) {
      ...
      return this.db.user.session.get().then(function (session) {
        return _this.db.transaction.get({ 'shipment._id': session.account._id });
        ...
        _this.groups = Object.keys(groups).map(function (key) {
          return groups[key];
        });

        _this.select(groups[params.id] || _this.groups[0]);
      }).catch(console.log);
    };

    inventory.prototype.scrollGroups = function scrollGroups($event) {
      var index = this.groups.indexOf(this.group);
      var last = this.groups.length - 1;

      if ($event.which == 38) this.select(this.groups[index > 0 ? index - 1 : last]);

      if ($event.which == 40) this.select(this.groups[index < last ? index + 1 : 0]);
    };

    inventory.prototype.select = function select(group) {
      var _this2 = this;

      if (group) {
        this.group = group;
        var drugId = group.transactions[0].drug._id;
        this.router.navigate('inventory/' + drugId, { trigger: false });
        this.db.drug.get({ _id: drugId }).then(function (drugs) {
          return _this2.image = drugs[0].image;
        });
      } else this.group = { transactions: [] };
    ...
    inventory.prototype.toggleRepack = function toggleRepack() {
      this.mode = !this.mode;
      this.repack = this.group.transactions.map(function (s) {
        return s.qty.from;
      });
      this.sumRepack();
      return true;
    ...
    inventory.prototype.sumGroup = function sumGroup() {
      this.group.total = this.group.transactions.reduce(function (a, b) {
    ...
    inventory.prototype.saveTransaction = function saveTransaction(transaction) {
      ...
      this.db.transaction.put(transaction).catch(function (e) {
        return _this3.snackbar.show({
          message: 'Transaction with exp ' + transaction.exp[_this3.role.account] + ' and qty ' + transaction.qty[_this3.role.account] + ' could not be saved'
        });
      });
    };
    ...
    inventory.prototype.repackage = function repackage() {
      ...
      for (var i = this.repack.length - 1; i >= 0; i--) {
        ...
        var src = this.group.transactions[i];
        ...
        if (src.qty.from == qty) {
          ...
          this.group.transactions.splice(i, 1);
...
define('views/shipments',['exports', 'aurelia-framework', 'aurelia-router', 'libs/pouch', 'aurelia-http-client', 'libs/csv'], function (exports, _aureliaFramework, _aureliaRouter, _pouch, _aureliaHttpClient, _csv) {
  ...
  var shipments = exports.shipments = (_dec = (0, _aureliaFramework.inject)(_pouch.Db, _aureliaRouter.Router, _aureliaHttpClient.HttpClient), _dec(_class = function () {
    function shipments(db, router, http) {
      ...
      this.csv = new _csv.Csv(['drug._id'], ['qty.from', 'qty.to', 'exp.from', 'exp.to', 'verifiedAt']);
      this.db = db;
      this.drugs = [];
      this.router = router;
      this.http = http;
    ...
    shipments.prototype.setTransactions = function setTransactions(shipmentId) {
      ...
      this.db.transaction.get({ 'shipment._id': shipmentId }).then(function (transactions) {
        ...
        for (var i in _this2.transactions) {
          _this2.isChecked.push(!!_this2.transactions[i].verifiedAt);
        }
      }).catch(console.log);
    };
    ...
    shipments.prototype.moveTransactionsToShipment = function moveTransactionsToShipment(shipment) {
      ...
      Promise.all(this.transactions.map(function (transaction, i) {
        if (!_this4.isChecked[i] || transaction.verifiedAt) return;
    ...
    shipments.prototype.qtyShortcuts = function qtyShortcuts($event, $index) {
      ...
      if ($event.which == 13) return document.querySelector('md-autocomplete input').focus();
      ...
      if (donorDelete || doneeDelete) {
        this.db.transaction.delete(transaction).then(function (_) {
          ...
          _this6.isChecked.splice($index, 1);
    ...
    shipments.prototype.autoCheck = function autoCheck($index) {
      ...
      var minQty = qty >= (+ordered.minQty || 10);
      var minExp = exp ? new Date(exp) - Date.now() >= (ordered.minDays || 120) * 24 * 60 * 60 * 1000 : !ordered.minDays;
      var isChecked = this.isChecked[$index] || false;

      if ((minQty && minExp) == isChecked) return console.log('minQty', minQty, qty, 'minExp', minExp, exp);

      this.manualCheck($index);
      this.isChecked[$index] = !isChecked;
      if (this.isChecked[$index]) this.snackbar.show(ordered.message || 'Drug is ordered');
    };
    ...
    shipments.prototype.saveInventory = function saveInventory() {
      ...
      Promise.all(this.diffs.map(function (i) {
        return _this7.db.transaction.verified[method]({ _id: _this7.transactions[i]._id }).then(function (_) {
          ...
          _this7.isChecked[i] = !!_this7.transactions[i].verifiedAt;
    ...
    shipments.prototype.search = function search() {
      ...
      if (term.length < 3) return this.drugs = [];
  ...
  var valueValueConverter = exports.valueValueConverter = function () {
    ...
    valueValueConverter.prototype.toView = function toView() {
      ...
      return transactions.reduce(function (a, b) {
        var price = b.drug.price && (b.drug.price.nadac || b.drug.price.goodrx) || 0;
