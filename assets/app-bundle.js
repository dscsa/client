...
define('views/inventory',['exports', 'aurelia-framework', '../libs/pouch', 'aurelia-router', '../libs/csv', '../resources/helpers'], function (exports, _aureliaFramework, _pouch, _aureliaRouter, _csv, _helpers) {
  ...
  var inventory = exports.inventory = (_dec = (0, _aureliaFramework.inject)(_pouch.Db, _aureliaRouter.Router), _dec(_class = function () {
    ...
    inventory.prototype.activate = function activate(params) {
      ...
      this.db.transaction.get({ pending: true }).then(function (pending) {
        _this.pending = {};
        for (var _iterator = pending, _isArray = Array.isArray(_iterator), _i = 0, _iterator = _isArray ? _iterator : _iterator[Symbol.iterator]();;) {
          var _ref;
          ...
          _this.pending[createdAt] = _this.pending[createdAt] || [];
          _this.pending[createdAt].push(transaction);
        }
      });
    };
    ...
    inventory.prototype.updateNext = function updateNext(updateFn) {
      var _this4 = this;

      var _loop = function _loop(i) {
        var transaction = _this4.transactions[i];
        if (transaction.isChecked) {
          updateFn(transaction.next);
    ...
    inventory.prototype.pendInventory = function pendInventory() {
      ...
      this.updateNext(function (next) {
        return next.push({ pending: {}, createdAt: createdAt });
      });
    };
    ...
    inventory.prototype.dispenseInventory = function dispenseInventory() {
      var createdAt = new Date().toJSON();
      this.updateNext(function (next) {
        return next.push({ dispensed: {}, createdAt: createdAt });
      });
    };
    ...
    inventory.prototype.repackInventory = function repackInventory() {
      ...
      for (var i = 0; i < this.repack.vials; i++) {
        ...
        this.transactions.unshift(_transaction);
        this.db.transaction.post(_transaction).then(function (transaction) {
          return _this5.updateNext(function (next) {
            next.push({ transaction: { _id: transaction._id }, createdAt: createdAt });
          });
        }).catch(function (err) {
          console.error(err);
          _this5.snackbar.show('Transaction could not be added: ' + err.reason);
          _this5.transactions.shift();
        });
      }
    };
    ...
    inventory.prototype.openMenu = function openMenu($event) {
      ...
      for (var _iterator4 = this.transactions, _isArray4 = Array.isArray(_iterator4), _i4 = 0, _iterator4 = _isArray4 ? _iterator4 : _iterator4[Symbol.iterator]();;) {
        ...
        if (_transaction2.isChecked) {
          this.repack.qty += _transaction2.qty.to;
          this.repack.exp = this.repack.exp < _transaction2.exp.to ? this.repack.exp : _transaction2.exp.to;
        }
      }
      this.setRepackVials();
    };
...
define('views/public',['exports', 'aurelia-framework', '../libs/pouch', '../resources/helpers'], function (exports, _aureliaFramework, _pouch, _helpers) {
  'use strict';
  ...
  var inventory = exports.inventory = (_dec = (0, _aureliaFramework.inject)(_pouch.Db), _dec(_class = function inventory(db) {
    var _this = this;
    ...
    this.db.transaction.get({ inventory: "sum" }).then(function (inventory) {
      return _this.inventory = inventory;
    });
    this.canActivate = _helpers.canActivate;
  }) || _class);
});
