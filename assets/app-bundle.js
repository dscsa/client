...
define('resources/helpers',['exports', 'aurelia-router'], function (exports, _aureliaRouter) {
  ...
  function drugSearch() {
    ...
    return this._search = Promise.resolve(this._search).then(function (_) {
      return _this2.db.drug.get(/^[\d-]+$/.test(term) ? { ndc: term } : { generic: term });
    });
  }
  ...
  function canActivate(_, next, _ref2) {
    ...
    return this.db.user.session.get().then(function (session) {
      ...
      for (var _iterator = router.navigation, _isArray = Array.isArray(_iterator), _i = 0, _iterator = _isArray ? _iterator : _iterator[Symbol.iterator]();;) {
        var _ref3;
  ...
  var inventoryFilterValueConverter = exports.inventoryFilterValueConverter = function () {
    ...
    inventoryFilterValueConverter.prototype.toView = function toView() {
      ...
      for (var _iterator = transactions, _isArray = Array.isArray(_iterator), _i = 0, _iterator = _isArray ? _iterator : _iterator[Symbol.iterator]();;) {
        ...
        var repack = this.isRepacked(transaction);
  ...
  var drugs = exports.drugs = (_dec = (0, _aureliaFramework.inject)(_pouch.Db, _aureliaRouter.Router), _dec(_class = function () {
    ...
    drugs.prototype.activate = function activate(params) {
      ...
      return this.db.user.session.get().then(function (session) {
        ...
        if (params.id) return _this.db.drug.get({ _id: params.id }).then(function (drugs) {
    ...
    drugs.prototype.selectGroup = function selectGroup(group, autoselectDrug) {
      ...
      this.db.transaction.get({ generic: group.name, inventory: "sum" }).then(function (inventory) {
        _this3.inventory = inventory[0].qty;
      });
    ...
    drugs.prototype.exportOrdered = function exportOrdered() {
      var _this6 = this;
      ...
      var _loop = function _loop(generic) {
        var order = _this6.account.ordered[generic];
        orders.push(_this6.db.transaction.get({ generic: generic, inventory: "sum" }).then(function (inventory) {
      ...
      for (var generic in this.account.ordered) {
        _loop(generic);
      }
    ...
    drugs.prototype.exportDrugs = function exportDrugs() {
      var _this7 = this;
      ...
      this.db.drug.get().then(function (drugs) {
        _this7.csv.unparse('Drugs.csv', drugs.map(function (drug) {
          return {
            ...
            ordered: _this7.account.ordered[drug.generic]
          };
        }));
      });
    };

    drugs.prototype.importDrugs = function importDrugs() {
      var _this8 = this;
    ...
    drugs.prototype.saveOrder = function saveOrder() {
      ...
      return this.db.account.put(this.account).catch(function (_) {
        _this9.snackbar.show('Order could not be saved: ' + (err.reason || err.message));
      });
    };
    ...
    drugs.prototype.addDrug = function addDrug() {
      ...
      this.db.drug.post(this.drug).then(function (res) {
        ...
        _this10._savingDrug = false;
        _this10.snackbar.show('Drug not added: ' + (err.reason || err.message));
      });
    };
    ...
    drugs.prototype.saveDrug = function saveDrug() {
      ...
      this.db.drug.put(this.drug).then(function (res) {
        if (_this11.group.name != _this11.drug.generic && _this11.group.drugs.length == 1 && _this11.account.ordered[_this11.group.name]) _this11.order();
  ...
  var inventory = exports.inventory = (_dec = (0, _aureliaFramework.inject)(_pouch.Db, _aureliaRouter.Router), _dec(_class = function () {
    ...
    inventory.prototype.activate = function activate(params) {
      ...
      this.db.user.session.get().then(function (session) {
        ...
        return _this.db.account.get({ _id: _this.account });
      }).then(function (accounts) {
        _this.ordered = accounts[0].ordered;
      });

      this.db.transaction.get({ pending: true }).then(function (transactions) {
        var pending = {};
        transactions = _this.sortTransactions(transactions);
        for (var _iterator = transactions, _isArray = Array.isArray(_iterator), _i = 0, _iterator = _isArray ? _iterator : _iterator[Symbol.iterator]();;) {
          var _ref;
        ...
        _this.pending = Object.keys(pending).map(function (key) {
          return pending[key];
        });
      });
    };
    ...
    inventory.prototype.selectGroup = function selectGroup(group) {
      ...
      this.router.navigate('inventory?' + (0, _aureliaFramework.buildQueryString)(group), { trigger: false });
      this.db.transaction.get(Object.assign({ inventory: true }, group), { limit: this.limit }).then(function (transactions) {
        if (transactions.length == _this2.limit) _this2.snackbar.show('Displaying first 100 results');

        _this2.term = group.generic || group.location || group.exp;
        _this2.group = group;
        _this2.pendingIndex = null;
        transactions = _this2.sortTransactions(transactions);
        _this2.setTransactions(transactions);
      }).catch(console.log);
    };

    inventory.prototype.selectPending = function selectPending($index) {
      var transactions = this.pending[$index];
    ...
    inventory.prototype.sortTransactions = function sortTransactions(transactions) {
      ...
      return transactions.sort(function (a, b) {
        ...
        var aBin = a.location || '';
        var bBin = b.location || '';
        var aPack = _this3.isRepacked(a);
        var bPack = _this3.isRepacked(b);
    ...
    inventory.prototype.search = function search() {
      ...
      if (/[A-Z][0-9]{3}/.test(this.term)) return this.selectGroup({ location: this.term });
    ...
    inventory.prototype.updateNextOfSelected = function updateNextOfSelected(updateFn) {
      var _this5 = this;
      ...
      var _loop = function _loop(i) {
        var transaction = _this5.transactions[i];
        ...
        _this5.db.transaction.put(transaction).catch(function (err) {
          transaction.next.pop();
          _this5.transactions.splice(i, 0, transaction);
          _this5.snackbar.show('Error removing inventory: ' + (err.reason || err.message));
        });
      };
    ...
    inventory.prototype.unpendInventory = function unpendInventory() {
      this.updateNextOfSelected(function (_) {
        return [];
      });
    };

    inventory.prototype.pendInventory = function pendInventory() {
      var selectedTransactions = this.updateNextOfSelected(function (_) {
        return [{ pending: {} }];
      });
    ...
    inventory.prototype.repackInventory = function repackInventory() {
      ...
      Promise.all(all).then(function (all) {
        ...
        (_transactions = _this6.transactions).unshift.apply(_transactions, all);
        _this6.updateNextOfSelected(function (_) {
        ...
        console.error(err);
        _this6.snackbar.show('Transactions could not repackaged: ' + err.reason);
      });
    };
    ...
    inventory.prototype.openMenu = function openMenu($event) {
      ...
      for (var _iterator5 = this.transactions, _isArray5 = Array.isArray(_iterator5), _i5 = 0, _iterator5 = _isArray5 ? _iterator5 : _iterator5[Symbol.iterator]();;) {
        ...
        if (_transaction.isChecked) {
          ...
          this.repack.exp = this.repack.exp < _transaction.exp.to ? this.repack.exp : _transaction.exp.to;
        }
      }
      this.setRepackVials();
    };
    ...
    inventory.prototype.exportCSV = function exportCSV() {
      ...
      this.db.transaction.get({ inventory: true }).then(function (inventory) {
        _this7.csv.unparse(name, inventory.map(function (row) {
    ...
    inventory.prototype.importCSV = function importCSV() {
      ...
      this.csv.parse(this.$file.files[0]).then(function (parsed) {
        return Promise.all(parsed.map(function (transaction) {
          ...
          return _this8.db.drug.get({ _id: transaction.drug._id }).then(function (drugs) {
        ...
        var _loop2 = function _loop2(i) {
          chain = chain.then(function (_) {
            ...
            args = args.map(function (row) {
              return _this8.db.transaction.post(row);
            });
            ...
            _this8.snackbar.show('Error Importing Inventory: ' + JSON.stringify(err));
          });
        };
        ...
        return _this8.snackbar.show('Imported Inventory Items');
      });
    };
...
define('views/join',['exports', 'aurelia-framework', 'aurelia-router', '../libs/pouch', 'aurelia-http-client', '../resources/helpers'], function (exports, _aureliaFramework, _aureliaRouter, _pouch, _aureliaHttpClient, _helpers) {
  'use strict';
...
define('views/login',['exports', 'aurelia-framework', 'aurelia-router', '../libs/pouch', '../resources/helpers'], function (exports, _aureliaFramework, _aureliaRouter, _pouch, _helpers) {
  'use strict';
...
define('views/public',['exports', 'aurelia-framework', '../libs/pouch', 'aurelia-router', '../resources/helpers'], function (exports, _aureliaFramework, _pouch, _aureliaRouter, _helpers) {
  'use strict';
  ...
  var inventory = exports.inventory = (_dec = (0, _aureliaFramework.inject)(_pouch.Db, _aureliaRouter.Router), _dec(_class = function () {
    ...
    inventory.prototype.activate = function activate(params) {
      ...
      this.db.account.get().then(function (accounts) {
        _this.accounts = accounts;
        _this.selectAccount(_this.accounts.reduce(reduce));
      });
    };

    inventory.prototype.selectAccount = function selectAccount($index) {
      var _this2 = this;

      this.account = this.accounts[$index]._id;
      this.router.navigate('inventory/' + this.account, { trigger: false });
      this.db.transaction.get({ inventory: "sum", account: this.account }).then(function (inventory) {
        return _this2.inventory = inventory;
      });
    };
...
define('views/routes',['exports'], function (exports) {
  'use strict';
  ...
  var App = exports.App = function () {
    ...
    App.prototype.configureRouter = function configureRouter(config, router) {
      ...
      config.title = 'SIRUM';
      config.map([{ route: 'inventory/:id', moduleId: 'views/public', title: 'Inventory' }, { route: 'login', moduleId: 'views/login', title: 'Login', nav: true }, { route: 'join', moduleId: 'views/join', title: 'Join', nav: true }, { route: 'inventory', moduleId: 'views/inventory', title: 'Inventory', nav: true, roles: ["user"] }, { route: ['shipments', 'shipments/:id', ''], moduleId: 'views/shipments', title: 'Shipments', nav: true, roles: ["user"] }, { route: ['drugs', 'drugs/:id'], moduleId: 'views/drugs', title: 'Drugs', nav: true, roles: ["user"] }, { route: ['records', 'records/:id'], moduleId: 'views/records', title: 'Records', nav: true, roles: ["user"] }, { route: 'account', moduleId: 'views/account', title: 'Account', nav: true, roles: ["user"] }]);
    };
...
define('views/shipments',['exports', 'aurelia-framework', 'aurelia-router', '../libs/pouch', 'aurelia-http-client', '../libs/csv', '../resources/helpers'], function (exports, _aureliaFramework, _aureliaRouter, _pouch, _aureliaHttpClient, _csv, _helpers) {
  ...
  var shipments = exports.shipments = (_dec = (0, _aureliaFramework.inject)(_pouch.Db, _aureliaRouter.Router, _aureliaHttpClient.HttpClient), _dec(_class = function () {
    ...
    shipments.prototype.addTransaction = function addTransaction(drug, transaction) {
      ...
      var order = this.getOrder(transaction);
      var isPharMerica = /pharmerica.*/i.test(this.shipment.account.from.name);

      order && this.db.transaction.get({ generic: drug.generic, inventory: "sum" }).then(function (inventory) {
