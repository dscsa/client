  ...
  function getHistory(id) {
    ...
    return this.db.transaction.history.get(id).then(function (_history) {
      ...
      return JSON.stringify(_history.reverse(), function (k, v) {
        ...
        var date = 'Date ' + v._id.slice(2, 10);
        var qty = 'Quantity ' + (v.qty.to || v.qty.from);
        var icon = "<a href='" + href + "'>" + v.type + " <i class='material-icons' style='font-size:12px; vertical-align:text-top; padding-top:1px'>exit_to_app</i></a>";
        var toName = '';
        var toStreet = '';
        var toAddress = '';
        var tracking = '';
        ...
        return pad(fromName) + pad(toName) + icon + "<br>" + pad(fromStreet) + pad(toStreet) + date + '<br>' + pad(fromAddress) + pad(toAddress) + qty + tracking;
  ...
  var drugs = exports.drugs = (_dec = (0, _aureliaFramework.inject)(_pouch.Pouch, _aureliaRouter.Router), _dec(_class = function () {
    ...
    drugs.prototype.selectGroup = function selectGroup(group, autoselectDrug) {
      ...
      this.db.transaction.query('inventory', { startkey: [this.account._id, group.name], endkey: [this.account._id, group.name + '\uFFFF'] }).then(function (inventory) {
        ...
        console.log('inventory', _this3.qtyBinned, _this3.qtyRepacked);
      });
  ...
  var inventory = exports.inventory = (_dec = (0, _aureliaFramework.inject)(_pouch.Pouch, _aureliaRouter.Router), _dec(_class = function () {
    ...
    inventory.prototype.repackInventory = function repackInventory() {
      ...
      Promise.all(newTransactions).then(function (newTransactions) {
        ...
        var label = ['<p style="page-break-after:always;">', '<strong>' + _this10.transactions[0].drug.generic + '</strong>', 'Ndc ' + _this10.transactions[0].drug._id, 'Exp ' + _this10.repack.exp.slice(0, 7), 'Bin ' + _this10.repack.bin, 'Qty ' + _this10.repack.vialQty, 'Pharmacist ________________', '</p>'];
