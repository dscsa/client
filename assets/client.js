  ...
  var drugs = exports.drugs = (_dec = (0, _aureliaFramework.inject)(_pouch.Pouch, _aureliaRouter.Router), _dec(_class = function () {
    ...
    drugs.prototype.search = function search() {
      ...
      return this.drugSearch().then(function (drugs) {
        ...
        console.log(drugs.length, _this4.groups.length, _this4.groups, _this4.account.ordered);
      });
    };
    ...
    drugs.prototype.order = function order() {
      ...
      if (this.account.ordered[this.group.generic]) {
        this.drawer.ordered = this.drawer.ordered.filter(function (generic) {
          return generic != _this5.group.generic;
        });
    ...
    drugs.prototype.saveAccount = function saveAccount() {
      ...
      return this.db.account.put(this.account).catch(function (_) {
        console.log('after saveAccount()', _this8.group.generic, _this8.drug.generic);
    ...
    drugs.prototype.saveDrug = function saveDrug() {
      ...
      this.db.drug.put(this.drug).then(function (res) {
        if (_this10.group.generic != _this10.drug.generic && _this10.group.drugs.length == 1 && _this10.account.ordered[_this10.group.generic]) _this10.order();
...
define('client/src/views/inventory',['exports', 'aurelia-framework', '../libs/pouch', 'aurelia-router', '../libs/csv', '../resources/helpers'], function (exports, _aureliaFramework, _pouch, _aureliaRouter, _csv, _helpers) {
  ...
  var inventory = exports.inventory = (_dec = (0, _aureliaFramework.inject)(_pouch.Pouch, _aureliaRouter.Router), _dec(_class = function () {
    ...
    inventory.prototype.search = function search() {
      ...
      if (this.isBinned(this.term)) return this.selectTerm('binned', this.term);

      if (this.isRepacked(this.term)) return this.selectTerm('repacked', this.term);

      if (this.isExpired(this.term)) return this.selectTerm('expired', this.term, true);

      this.drugSearch().then(function (drugs) {
        console.log(drugs.length, _this3.account.ordered, _this3.account);
        _this3.groups = _this3.groupDrugs(drugs, _this3.account.ordered);
        console.log(drugs.length, _this3.groups.length, _this3.groups);
      });
    };

    inventory.prototype.isBinned = function isBinned(term) {
      return (/^[A-Za-z][0-9]{3}$/.test(term)
      );
    };

    inventory.prototype.isRepacked = function isRepacked(term) {
      return (/^[A-Za-z][0-9]{2}$/.test(term)
      );
    };

    inventory.prototype.isExpired = function isExpired(term) {
    ...
    inventory.prototype.selectInventory = function selectInventory(type, key, limit) {
      ...
      if (type == 'binned') {
        ...
        var _currentDate = this.currentDate(1),
            year = _currentDate[0],
    ...
    inventory.prototype.unpendInventory = function unpendInventory() {
      ...
      this.updateSelected(function (transaction) {
        ...
        return term ? _this6.selectTerm('drug', term) : _this6.term = '';
      });
    };
    ...
    inventory.prototype.repackInventory = function repackInventory() {
      ...
      this.db.transaction.bulkDocs(newTransactions).then(function (rows) {
        ...
        if (errors.length) {
          ...
          return _this7.snackbar.show('Repack error ' + errors.join(', '));
        }
        ...
        console.error(err);
        _this7.snackbar.show('Transactions could not repackaged: ' + err.reason);
      });
    };
    ...
    inventory.prototype.saveAndReconcileTransaction = function saveAndReconcileTransaction(transaction) {
      ...
      this.db.transaction.get(transaction._id).then(function (repack) {
        ...
        if (!qtyChange) {
          return _this8.saveTransaction(transaction);
        }

        _this8.transactions = _this8.transactions.slice();

        if (transaction.isChecked) _this8.filter.checked.qty += qtyChange;

        console.log('saveAndReconcileTransaction', qtyChange, transaction.qty.to, repack.qty.to);

        return _this8.db.transaction.query('next.transaction._id', { key: [_this8.account._id, transaction._id], include_docs: true }).then(function (res) {
          ...
          return _this8.db.transaction.get(excess).then(function (excess) {
            excess.qty.to -= qtyChange;

            if (excess.qty.to < 0) {
              transaction.qty.to = repack.qty.to;
              return _this8.snackbar.show('Cannot set repack qty to be more than qty orginally repacked, ' + (repack.qty.to + excess.qty.to + qtyChange));
            }

            _this8.db.transaction.put(excess);
            return _this8.saveTransaction(transaction);
          });
        });
      });
    ...
    inventory.prototype.currentDate = function currentDate(addMonths) {
      var minExp = new Date();
    ...
    inventory.prototype.exportCSV = function exportCSV() {
      ...
      var _currentDate2 = this.currentDate(1),
  ...
  var inventoryFilterValueConverter = exports.inventoryFilterValueConverter = function () {
    ...
    inventoryFilterValueConverter.prototype.toView = function toView() {
      ...
      var defaultCheck = inventory.prototype.isBinned(term) || inventory.prototype.isRepacked(term) || inventory.prototype.isExpired(term);
