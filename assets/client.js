  ...
  var shopping = exports.shopping = (_dec = (0, _aureliaFramework.inject)(_pouch.Pouch, _aureliaRouter.Router), _dec(_class = function () {
    ...
    shopping.prototype.allBaskets = function allBaskets(generic) {
      ...
      for (var i = 0; i < this.shopList.length; i++) {
        if (this.shopList[i].extra.basketNumber.length > 1 && !~list_of_baskets.indexOf(this.shopList[i].extra.basketNumber) && (this.shopList[i].raw.drug.generic = generic)) list_of_baskets.unshift(this.shopList[i].extra.basketNumber);
      }
      return list_of_baskets.sort(function (a, b) {
        return a == _this7.shopList[_this7.shoppingIndex].extra.basketNumber;
      }).join(",");
    };
      ...
      if (this.getOutcome(this.shopList[this.shoppingIndex].extra) == 'missing' && this.shopList[this.shoppingIndex].extra.saved != 'missing') {
        ...
        this.db.account.picking['post']({ groupName: this.shopList[this.shoppingIndex].raw.next[0].pended.group, action: 'missing_transaction', ndc: this.shopList[this.shoppingIndex].raw.drug._id, generic: this.shopList[this.shoppingIndex].raw.drug.generic, qty: this.shopList[this.shoppingIndex].raw.qty.to }).then(function (res) {
          ...
          if (res.length > 0) {
            ...
            _this8.shopList[_this8.shoppingIndex].extra.saved = 'missing';

            for (var j = 0; j < res.length; j++) {

              var n = _this8.shoppingIndex - (_this8.shopList[_this8.shoppingIndex].extra.genericIndex.relative_index[0] - 1);
              if (n < 0) n = 0;
              var inserted = false;

              for (n; n < _this8.shopList.length; n++) {

                if (_this8.shopList[n].raw.drug.generic == res[j].raw.drug.generic) {
                  _this8.shopList[n].extra.genericIndex.relative_index[1]++;
                } else {
                  res[j].extra.genericIndex = { global_index: _this8.shopList[n - 1].extra.genericIndex.global_index, relative_index: [_this8.shopList[n - 1].extra.genericIndex.relative_index[0] + 1, _this8.shopList[n - 1].extra.genericIndex.relative_index[1]] };
                  _this8.shopList.splice(n, 0, res[j]);
                  inserted = true;
                  n = _this8.shopList.length;
                }
              }

              if (!inserted) {
                res[j].extra.genericIndex = { global_index: _this8.shopList[n - 1].extra.genericIndex.global_index, relative_index: [_this8.shopList[n - 1].extra.genericIndex.relative_index[0] + 1, _this8.shopList[n - 1].extra.genericIndex.relative_index[1]] };
                _this8.shopList.push(res[j]);
              }
            }
          } else {
