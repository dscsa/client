  ...
  var shopping = exports.shopping = (_dec = (0, _aureliaFramework.inject)(_pouch.Pouch, _aureliaRouter.Router), _dec(_class = function () {
    ...
    shopping.prototype.activate = function activate(params) {
      ...
      this.db.user.session.get().then(function (session) {
        ...
        if (params.groupName) {
          _this.db.account.picking.post({ groupName: params.groupName, action: 'group_info' }).then(function (result) {
            if (result.shopList) {
              _this.shopList = result.shopList;
              _this.groupData = result.groupData;
        ...
        } else _this.manageShoppingIndex();
      }).catch(function (err) {
    ...
    shopping.prototype.selectGroup = function selectGroup(groupName, isLocked, isLockedByCurrentUser) {
      ...
      this.db.account.picking.post({ groupName: groupName, action: 'load' }).then(function (res) {
        ...
        _this5.basketSaved = hasBasket;

        if (hasBasket) {
          _this5.addBasketToShoppingList(res.groupData.basket.fullBasket);
        }
      }).catch(function (err) {
    ...
    shopping.prototype.saveBasketNumber = function saveBasketNumber() {
      ...
      this.db.account.picking.post({ groupName: this.groupName, basket: basket, action: 'save_basket_number' }).then(function (res) {
        if (res && res[0] === 'success') {
          _this7.basketSaved = true;
        }
      });
    ...
    shopping.prototype.setShoppingIndex = function setShoppingIndex(index) {
      ...
      var goToStep = function goToStep() {
        _this10.shoppingIndex = index - 1;
        _this10.basketSaved = _this10.groupData.basket && _this10.groupData.basket.number;
        console.log('basket save ', _this10.basketSaved);

        if (index < 0 && _this10.basketSaved) {
          _this10.shoppingIndex = 1;
        }

        if (_this10.basketSaved) {
          var basket = _this10.groupData.basket;
          _this10.addBasketToShoppingList(basket);
        }
        if (_this10.shoppingIndex === _this10.shopList.length - 1) {
          _this10.setNextToSave();
        } else {
          _this10.setNextToNext();
        }

        _this10.formComplete = _this10.shopList[_this10.shoppingIndex].extra.fullBasket && _this10.someOutcomeSelected(_this10.shopList[_this10.shoppingIndex].extra.outcome);
        history.pushState(null, null, '#/picking/' + _this10.groupName + '/step/' + (_this10.shoppingIndex + 1));
      };

      if (!this.shopList.length) {

        this.db.account.picking.post({ groupName: this.groupName, action: 'load' }).then(function (res) {
          _this10.groupData = res.groupData;
          _this10.shopList = res.shopList;
          _this10.initializeShopper();
          goToStep();
        });
      } else {
        goToStep();
