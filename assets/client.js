...
define('client/src/views/inventory',['exports', 'aurelia-framework', '../libs/pouch', 'aurelia-router', '../libs/csv', '../resources/helpers'], function (exports, _aureliaFramework, _pouch, _aureliaRouter, _csv, _helpers) {
  ...
  var inventory = exports.inventory = (_dec = (0, _aureliaFramework.inject)(_pouch.Pouch, _aureliaRouter.Router), _dec(_class = function () {
    ...
    inventory.prototype.setTransactions = function setTransactions() {
      ...
      if (~['M00', 'T00', 'W00', 'R00', 'F00'].indexOf(this.term)) transactions = transactions.sort(function (a, b) {
    ...
    inventory.prototype.selectInventory = function selectInventory(type, key, limit) {
      ...
      var setTransactions = function setTransactions(res) {
        ...
        for (var _iterator4 = res_items, _isArray4 = Array.isArray(_iterator4), _i4 = 0, _iterator4 = _isArray4 ? _iterator4 : _iterator4[Symbol.iterator]();;) {
          var _ref4;
          ...
          var _isOrdered2 = _this8.account.ordered[doc.drug.generic];

          var exp = doc.exp.to || doc.exp.from || oneMonthFromNow;

          console.log('destruction highlighting', type, exp.slice(0, 7), oneMonthFromNow, type == 'bin' && exp.slice(0, 7) <= oneMonthFromNow, doc, _isOrdered2);
          if (type == 'bin' && exp.slice(0, 7) <= oneMonthFromNow) {
            doc.highlighted = _this8.destroyedColor(_isOrdered2 ? _isOrdered2.destroyedMessage : '');
          }

          if (doc.next && !doc.next.length) {
            docs.push(doc);
          } else if (type == 'bin' && doc.next && doc.next[0].pended && doc.next[0].picked) {
            docs.push(doc);
          } else {
      ...
      var incrementString = function incrementString(s) {
      ...
      if (type == 'bin') {
        this.db.transaction.find({
          selector: {
            $and: [{ bin: { $gte: key } }, { bin: { $lt: key.slice(0, -1) + incrementString(key[key.length - 1]) } }, { bin: { $regex: '^' + key } }]
          }
