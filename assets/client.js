  ...
  var drugs = exports.drugs = (_dec = (0, _aureliaFramework.inject)(_pouch.Pouch, _aureliaRouter.Router), _dec(_class = function () {
    ...
    drugs.prototype.selectGroup = function selectGroup(group, autoselectDrug) {
      ...
      this.db.transaction.query('inventory', { startkey: [this.account._id, group.name, indate], endkey: [this.account._id, group.name, {}] }).then(function (inventory) {
        ...
        _this3.indateInventory = row ? row.value['qty.binned'] || 0 + row.value['qty.repacked'] || 0 : 0;
...
define('client/src/views/inventory',['exports', 'aurelia-framework', '../libs/pouch', 'aurelia-router', '../libs/csv', '../resources/helpers'], function (exports, _aureliaFramework, _pouch, _aureliaRouter, _csv, _helpers) {
  ...
  var inventory = exports.inventory = (_dec = (0, _aureliaFramework.inject)(_pouch.Pouch, _aureliaRouter.Router), _dec(_class = function () {
    ...
    inventory.prototype.pendInventory = function pendInventory(pendId) {
      var _this8 = this;
      ...
      this.updateSelected(function (transaction) {
        transaction.isChecked = false;
        transaction.next = [{ pending: { _id: _this8.newPendId }, createdAt: createdAt }];
    ...
    inventory.prototype.setPending = function setPending(transactions) {
      ...
      for (var _iterator2 = transactions, _isArray2 = Array.isArray(_iterator2), _i2 = 0, _iterator2 = _isArray2 ? _iterator2 : _iterator2[Symbol.iterator]();;) {
        ...
        var pendId = _transaction.next[0].pending._id || _transaction.next[0].createdAt.slice(5, 16).replace('T', ' ');

        this.pending[pendId] = this.pending[pendId] || {};
        this.pending[pendId][generic] = this.pending[pendId][generic] || [];
        this.pending[pendId][generic].push(_transaction);
        this.pending[pendId][generic].sort(this.sortPending.bind(this));
      }
    };
    ...
    inventory.prototype.saveAndReconcileTransaction = function saveAndReconcileTransaction(transaction) {
      ...
      this.db.transaction.get(transaction._id).then(function (repack) {
        ...
        return _this10.db.transaction.query('next.transaction._id', { key: [_this10.account, transaction._id], include_docs: true }).then(function (res) {
    ...
    inventory.prototype.setMatchingPends = function setMatchingPends(generic) {

      var matches = [];
      for (var pendId in this.pending) {
        if (generic in this.pending[pendId]) matches.push(pendId);
      }return matches;
    };
  ...
  var pendingFilterValueConverter = exports.pendingFilterValueConverter = function () {
    function pendingFilterValueConverter() {
      _classCallCheck(this, pendingFilterValueConverter);
    }
    ...
    pendingFilterValueConverter.prototype.toView = function toView() {
      ...
      for (var pendId in pending) {
        ...
        for (var generic in pending[pendId]) {
          if (~generic.indexOf(term)) genericMatches[generic] = pending[pendId][generic];
    ...
    return pendingFilterValueConverter;
  }();
});
