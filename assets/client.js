        ...
        if (next[0]) {
          if (key == 'shopped') {
            ...
            if (outcome == 'missing') {
              ...
              this.db.account.picking['post']({ groupName: arr_enriched_transactions[i].raw.next[0].pended.group, action: 'missing_transaction', generic: arr_enriched_transactions[i].raw.drug.generic, qty: arr_enriched_transactions[i].raw.qty.to }).then(function (res) {
                console.log(res);
                if (res.length > 0) {
                  _this5.shopList.push(res[0]);
    ...
    shopping.prototype.moveShoppingForward = function moveShoppingForward() {
      ...
      if (this.shoppingIndex == this.shopList.length - 1) {
        ...
        this.saveShoppingResults([this.shopList[this.shoppingIndex]], 'shopped').then(function (_) {
          if (_this6.shoppingIndex != _this6.shopList.length - 1) {
            _this6.moveShoppingForward();
          } else {
            _this6.resetShopper();
